...
const MIN_INSTALLMENT = 300;

function installmentsFor(freq: Frequency, weeks: number) {
  return freq === "daily" ? weeks * 7 : weeks;
}

function perInstallmentFor(price: number, freq: Frequency, weeks: number) {
  const count = installmentsFor(freq, weeks);
  return count > 0 ? Math.ceil(price / count) : 0;
}

function isValidOption(price: number, freq: Frequency, weeks: number) {
  return price > 0 && perInstallmentFor(price, freq, weeks) >= MIN_INSTALLMENT;
}

export function InstallmentCalculator({ className }: { className?: string }) {
  ...
  const phone = phones.find((p) => p.id === selectedPhoneId) ?? phones[0] ?? null;
  ...
  const weekOptions = WEEK_OPTIONS.map((w) => ({
    weeks: w,
    valid: isValidOption(price, freq, w),
  }));
  ...
  const installments = effectiveWeeks ? installmentsFor(freq, effectiveWeeks) : 0;
  const perInstallment = effectiveWeeks ? perInstallmentFor(price, freq, effectiveWeeks) : 0;
  const lastInstallment = installments > 0 ? price - perInstallment * (installments - 1) : 0;
  const hasRounding = installments > 1 && lastInstallment !== perInstallment;

  const dueDate = effectiveWeeks
    ? (() => {
        const d = new Date();
        d.setDate(d.getDate() + effectiveWeeks * 7);
        return d;
      })()
    : null;
    ...
    <div className={className}>
      <div className="rounded-3xl border border-border bg-card p-5 sm:p-7">
        <p className="inline-flex items-center gap-1.5 text-sm font-medium text-primary">
          <Sparkles className="size-3.5" /> ลองคำนวณดูก่อนได้
        </p>
        <h2 className="mt-1 text-2xl font-semibold tracking-tight">คำนวณค่าผ่อน</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          เลือกเครื่อง แล้วดูว่าถ้าผ่อนรายวันหรือรายสัปดาห์ ต้องจ่ายงวดละเท่าไหร่
        </p>
          ...
          <>
                ...
                <div className="mt-2 flex gap-2">
                  {weekOptions.map((o) => (
                    <button
                      ...
                      title={!o.valid ? `ยอดต่องวดจะต่ำกว่า ${MIN_INSTALLMENT} บาท` : undefined}
            ...
            {!hasAnyValidOption ? (
              <p className="mt-6 rounded-2xl border border-dashed border-border bg-background p-5 text-center text-sm text-muted-foreground">
                ราคาเครื่องนี้ต่ำเกินกว่าจะแบ่งจ่ายได้ครบตามเงื่อนไขขั้นต่ำงวดละ {money.format(MIN_INSTALLMENT)} แนะนำชำระเต็มจำนวน
              </p>
            ) : (
              <div className="relative mt-6 overflow-hidden rounded-2xl bg-gradient-brand p-6 text-primary-foreground">
                ...
                <div className="relative flex flex-wrap items-end justify-between gap-4">
                  <div>
                    <p className="text-sm text-primary-foreground/70">ผ่อนงวดละประมาณ</p>
                    <p className="mt-1 text-3xl font-semibold">{money.format(perInstallment)}</p>
                    <p className="mt-1 text-xs text-primary-foreground/70">
                      {installments} งวด · {freq === "daily" ? "ทุกวัน" : "ทุกสัปดาห์"}
            ...
            <div className="mt-5 flex gap-2.5 rounded-2xl border border-warning/30 bg-warning/10 p-4 text-xs leading-5 text-warning-foreground">
              ...
              <div className="space-y-1">
                <p>ยอดชำระแต่ละงวดต้องไม่ต่ำกว่า {money.format(MIN_INSTALLMENT)} และต้องชำระให้ครบภายในระยะเวลาที่เลือก นับจากวันที่ชำระงวดแรก</p>
                <p>หากชำระไม่ครบภายในกำหนด ทางร้านขอสงวนสิทธิ์ยึดเงินที่ชำระมาแล้วทั้งหมด และจะไม่คืนเงินในทุกกรณี</p>
                <p className="text-warning-foreground/70">
                  ตัวเลขด้านบนเป็นการประเมินเบื้องต้นเท่านั้น ไม่ใช่การยืนยันคำสั่งซื้อ กรุณายืนยันแผนผ่อนกับร้านอีกครั้งก่อนชำระ
                </p>
              </div>
            </div>
          </>
